...
if (isset($_POST["seminarPrezent"])) {

    $id = $_SESSION["userId"];
    $today = getdate();
    $todayDate = $today["mday"] . $today["mon"] . $today["year"] . " ";

    $sql01 = "SELECT * FROM presencesSeminar WHERE userId='$id'";
    $result01 = mysqli_query($conn, $sql01);
    $row01 = mysqli_fetch_assoc($result01);

    $todayDateActual = $row01["prezent"] . $todayDate;

    $sql = "UPDATE presencesSeminar SET prezent='$todayDateActual' WHERE userId='$id'";
